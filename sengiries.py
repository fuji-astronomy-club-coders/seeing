...
def sengiri_X2_justOUTside_edgepoints(    
    ...
    if file.endswith(".tiff" or ".tif"):
        ...
        img_UNC= cv2.imread(file,cv2.IMREAD_UNCHANGED)
        pl=["L","R","T","B"]
        #縦倒し,逆順
        pldc={
            "L":(True,False),
        ...
        def plot_sample():
                #太陽像上のサンプルの位置をプロット
                fig, ax = plt.subplots()
                ax.imshow(img_UNC, cmap='magma')
                ax.scatter(Medge[0], Medge[1], color='cyan', label='MIN edge', s=10)
                ax.plot([sampe[0],sampe[1]], [sampe[2],sampe[3]], color='green', label='sample', alpha=0.5)
                ax.add_artist(plt.Circle((xc, yc), r, color='blue', fill=False, label='MIN2 circle', linestyle="--"))
        ...
        for gaps in range(-gap , gap):          
            ...
            for place in pl:
                try:
                    ...
                    sample=img_UNC[sampe[0]:sampe[1], sampe[2]:sampe[3]]
                    sample=np.array(np.ravel(sample))
