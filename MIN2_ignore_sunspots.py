...
def fit_circle(spots):#縁の点のサンプルを受け取って円のstatusを返す。
    if len(spots)<3:
        raise(f"点不足,nが小さすぎるか、黒点が多すぎ{show_circle(spots,False)}")
...
def MIN2_ignore_sunspots(readed_img,n=10,light_threshold=50,limb_wigth=24,show=False):
    """
    img:読み込んだ画像を渡してください
    n:画像格子の分割数
    ...
    #print("first circle");show_circle(spots,(cx,cy,r))#一回目の円を表示debug用
    ...
    #print("only outside circle");show_circle(outside_spots,(cxo,cyo,ro))#外側の点だけで作成した円を表示
